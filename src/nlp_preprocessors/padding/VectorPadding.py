...
class VectorPadding:
    ...
    def __call__(self, inputs: list[np.ndarray]):
        """
        inputs: (batch_size, *vectors_num, vector_size)
        *Dimension to pad
        ...
        # Initial
        returns = {}
        vectors = np.zeros([batch_size, padding_length, vector_size], dtype=float)
        ...
        for i in range(batch_size):
            vectors[i, :len(inputs[i])] = inputs[i][:padding_length]
            if self.return_padding_mask:
